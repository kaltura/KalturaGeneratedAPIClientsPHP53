 ...
 * @subpackage Client
 */
class DataService extends \Kaltura\Client\ServiceBase
{
	 ...
	 */
	function add(\Kaltura\Client\Type\DataEntry $dataEntry)
	{
		$kparams = array();
		$this->client->addParam($kparams, "dataEntry", $dataEntry->toParams());
		$this->client->queueServiceActionCall("data", "add", "KalturaDataEntry", $kparams);
	...
	function get($entryId, $version = -1)
	{
		...
		$this->client->queueServiceActionCall("data", "get", "KalturaDataEntry", $kparams);
		if ($this->client->isMultiRequest())
			return $this->client->getMultiRequestResult();
		$resultXml = $this->client->doQueue();
		$resultXmlObject = new \SimpleXMLElement($resultXml);
	 ...
	 */
	function listAction(\Kaltura\Client\Type\DataEntryFilter $filter = null, \Kaltura\Client\Type\FilterPager $pager = null)
	{
		$kparams = array();
		if ($filter !== null)
		...
		$this->client->queueServiceActionCall("data", "list", "KalturaDataListResponse", $kparams);
		if ($this->client->isMultiRequest())
			return $this->client->getMultiRequestResult();
		$resultXml = $this->client->doQueue();
		$resultXmlObject = new \SimpleXMLElement($resultXml);
		$this->client->checkIfError($resultXmlObject->result);
		$resultObject = \Kaltura\Client\ParseUtils::unmarshalObject($resultXmlObject->result, "KalturaDataListResponse");
		$this->client->validateObjectType($resultObject, "\\Kaltura\\Client\\Type\\DataListResponse");
		return $resultObject;
	}
